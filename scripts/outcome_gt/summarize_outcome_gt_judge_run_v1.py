...
def summarize(pass_id: str, run_id: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": "outcome_gt_judge_run_diagnostic.v1",
        "pass_id": pass_id,
        "run_id": run_id,
        "raw_rows": len(rows),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Summarize Outcome GT judge-run operational failures without printing response-bearing raw content.")
