...
REQUEST_SCHEMA_PATH = ROOT / "schemas/outcome_gt_judge_request_v2.schema.json"
...
def request_id(pass_id: str, assignment_item_id: str) -> str:
    ...
    suffix = hashlib.sha256(
        f"outcome-gt-judge-request-v2:{pass_id}:{assignment_item_id}:{PROMPT_SHA256}".encode("utf-8")
    ).hexdigest()[:16].upper()
...
def materialize_one(pass_id: str, output_root: Path) -> dict[str, Any]:
    ...
    for assignment in assignments:
        row = {
            "schema_version": "outcome_gt_judge_request.v2",
    ...
    request_path = output_dir / "judge_requests_1746_v2.jsonl"
    request_manifest_path = output_dir / "judge_requests_1746_v2_manifest.json"
    request_bytes = canonical_jsonl_bytes(rows)
    request_manifest = {
        "schema_version": "outcome_gt_judge_request_manifest.v2",
        "pass_id": pass_id,
        "prompt_version": PROMPT_VERSION,
...
def main() -> int:
    ...
    print(json.dumps({
        "status": "JUDGE_REQUESTS_V2_READY",
        "total_calls": total,
        "passes": manifests,
