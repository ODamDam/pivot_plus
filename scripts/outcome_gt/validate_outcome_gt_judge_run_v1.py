...
PROVENANCE_SCHEMA = ROOT / "schemas/outcome_gt_annotator_provenance_v1.schema.json"
...
def main() -> int:
    ...
    if not provenance_path.exists() or not decisions_path.exists() or not checkpoint_path.exists():
        raise FileNotFoundError("judge run is missing provenance, decisions, or checkpoint")

    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    errors = list(load_validator().iter_errors(provenance))
    if errors:
        raise ValueError(f"annotator provenance schema failure: {errors[0].message}")
    ...
    request_manifest_path = REQUEST_ROOT / args.pass_id / "judge_requests_1746_v1_manifest.json"
    ...
    if provenance["request_sha256"] != sha256_file(request_path):
        raise ValueError("judge request provenance mismatch")
